...
reclamacoes_l = reclamacoes_l %>% 
     group_by(lexico) %>% 
    mutate(polaridade_normalizada = case_when(polaridade < -6 ~ 5, polaridade < -1 ~ 4 , polaridade < 4 ~ 3, polaridade < 9 ~ 2, polaridade >= 9 ~ 1))
```
...
#ggpairs(reclamacoes_l %>% select(erro,insatisfacao))
lm1 = lm(reclamacoes_l$lexico.dummy~reclamacoes_l$erro)
tidy(lm1, conf.int = TRUE, conf.level = 0.95)
glance(lm1)
reclamacoes_l1 = reclamacoes_l %>% add_predictions(lm1)
summary(reclamacoes_l1)



reclamacoes_l1 %>% 
  ggplot(aes(x = `lexico.dummy`)) + 
